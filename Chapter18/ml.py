...
def day_of_year_num(dts):
    return dts.astype("datetime64[D]").view("int64") - dts.astype(
        "datetime64[Y]"
    ).astype("datetime64[D]").view("int64")


def time_of_day_num(dts):
    return dts.astype("datetime64[s]").view("int64") - dts.astype(
        "datetime64[D]"
    ).astype("datetime64[s]").view("int64")
    ...
    def transform(self, X, y=None, groups=None, cols=None):
        ...
        if str(type(X)) == "<class 'numpy.ndarray'>":
            for i in self.indices:
                dates = X[:, i]
                X = np.delete(X, i, 1)
                newcols = np.array(shape=(X.shape[0], 3), dtype=np.int64)

                newcols[:, 0] = day_of_week_num(dates)
                newcols[:, 1] = day_of_year_num(dates)
                newcols[:, 1] = time_of_day_num(dates)
